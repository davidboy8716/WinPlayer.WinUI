...
4. 若改动涉及界面，请附上截图，并把截图放到 `docs/screenshots/`。
   **截图前请注意**：本程序会把媒体文件夹树与播放列表画在界面上，截图很容易连带
   真实文件夹名、文件名或完整路径一起公开。请用不含个人内容的数据截图，
   或在使用前自行确认截图中没有需要保密的路径与文件名。
5. 不要在 PR 中夹带与主题无关的格式化改动。
