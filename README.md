...
<p align="center">
  <img src="docs/screenshots/main.jpg" alt="WinPlayer.WinUI 主界面" width="760">
</p>

选项窗口：独立窗口，不随主窗口缩小而被裁切（图中为普通模式；标题栏右侧的模式标记会显示当前是普通还是隐私模式）。
...
**选项窗口整体没有毛玻璃效果**：它的窗口背景是一层不透明的主题画刷（`SettingsRoot` 的
`ApplicationPageBackgroundThemeBrush`），把底下的内容全盖住了。

> 值得注意的是：两个窗口的 XAML 其实都声明了
> `<Window.SystemBackdrop><MicaBackdrop Kind="BaseAlt" /></Window.SystemBackdrop>`，
> 但**都看不到效果**——主窗口的根 Grid 是 `Background="Black"`，选项窗口的根 Grid 是不透明的
> `ApplicationPageBackgroundThemeBrush`，各自把整个客户区铺满，Mica 没有可显示的地方。

### 与毛玻璃有关的选项

「毛玻璃模糊强度」与「色调不透明度」两项只作用于**主窗口面板**那一层 Win2D 毛玻璃，
既不影响选项窗口，也不能给选项窗口加上毛玻璃。

「直通呈现」（见 [HDR 与杜比视界支持](#hdr-与杜比视界支持)）会让主窗口面板不再采样视频，
即**这一层**毛玻璃失效。
...
- 视频画面作为窗口背景，标题栏、文件夹树、播放列表、控制栏均为悬浮毛玻璃面板（用 Win2D 采样背后的视频）
- 毛玻璃模糊强度与色调不透明度可调（只作用于主窗口面板这一层，见[界面的半透明效果](#界面的半透明效果)）
- 左侧媒体文件夹树与右侧播放列表，支持拖入文件或文件夹（异步扫描，不阻塞界面）
- 侧栏宽度可拖动调整，可独立显示 / 隐藏，带自动隐藏动画
- 底部控制栏可自动隐藏或固定显示，延迟与动画时长可调
- 毛玻璃模糊强度与色调不透明度可调
- 独立的选项窗口，导航分为「播放 / 界面 / 启动 / 日志」四页
...
| 容器内嵌位图字幕 | **由系统解码**，本程序取出系统给出的字幕帧后画在自己的叠加画布上（不解析码流） |
...
- **本项目字幕** —— 控制本程序是否绘制**自己产生**的字幕：文本字幕与外挂 `.sup`。
  片源内嵌的位图字幕由系统解码、本程序只负责呈现，不受它影响，要关它请用「关闭字幕」
- 引擎字幕轨列表 —— libmpv 引擎模式下列出引擎读到的字幕轨
- **关闭字幕** —— 关闭字幕；这是关掉片源内嵌位图字幕的入口
- **加载外挂字幕…**
...
| 设置 | 默认值 | 说明 |
| --- | --- | --- |
| 本项目字幕 | 显示 | 关闭后本程序不再绘制自己产生的字幕（文本浮层与外挂 `.sup`）；片源内嵌位图字幕由系统解码，不受该开关影响，要用「关闭字幕」关它 |
| 字幕字体 | Segoe UI | 文本字幕使用的字体 |
| 字幕基准字号 | 24 | 字幕文字大小，随播放区域缩放 |
| 字幕底部位置 | 17% | 文本字幕与播放区域底部的距离，按播放区域高度百分比计算 |
| 图片字幕上移 | 0% | 把图片字幕（内嵌位图、外挂 `.sup`）整体上移，按播放区域高度百分比计算。0 = 按片源给定位置；画面自带文字时可调大以便把两层分开 |
| 图片字幕只保留一行 | 显示整条提示 | 片源把一句话排成两行、或一条提示里放了两种语言时，只显示选中的那一行（上一行 / 下一行）。**分行的句子会少掉另一半**，默认关闭 |
...
| **直通呈现** | 「直通呈现」= 直通 | 关闭帧服务器自绘，把画面交回系统合成器输出，由系统处理 HDR | **面板级**毛玻璃采样不生效（悬浮面板不再采样视频）；图片字幕改由独立叠加层绘制 |
...
「选项 → 日志」中的 **HDR / 杜比诊断** 按钮会给出：当前是自绘还是直通呈现、系统给出的输出
