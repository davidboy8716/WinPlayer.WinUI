...
| 来源 | 支持方式 |
| --- | --- |
| 容器内嵌文本字幕 | 从媒体文件读出字幕轨，在字幕菜单中列出并切换 |
| 容器内嵌位图字幕 | 由本程序自行解码并绘制（PGS / 蓝光图形字幕这类以图片形式存储的字幕） |
| 外挂字幕文件 | 字幕菜单中的「加载外挂字幕…」，支持 `.srt`、`.ass`、`.ssa`、`.vtt`、`.ttml`、`.sup`、`.sub` |

> 字幕能否使用取决于片源里实际存在的字幕轨与编码格式。位图字幕的解码支持有范围限制，
> 遇到无法识别的内容时，可改用 libmpv 杜比视界引擎播放，由引擎自己渲染字幕。
...
| **直通呈现** | 「直通呈现」= 直通 | 关闭帧服务器自绘，把画面交回系统合成器输出，由系统处理 HDR | 毛玻璃采样不生效；图片字幕改由独立叠加层绘制 |
| **libmpv 杜比视界引擎** | 「libmpv 杜比视界引擎」= 启用 | 播放整体交给 libmpv（软件渲染 API + libplacebo 滤镜链），由 libplacebo 完成杜比视界重塑与色调映射 | 需自行提供 `libmpv-2.dll`；字幕改由引擎渲染 |
...
- 位图字幕由本程序自行解码绘制，支持范围有限；遇到无法识别的内容时，可改由 libmpv 引擎渲染。
